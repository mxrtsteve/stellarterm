...
const Validate = {
    ...
    memo(memoInput, type) {
        ...
        case 'MEMO_TEXT':
            if (Buffer.byteLength(input, 'utf8') > 28) {
                return resultError(`MEMO_TEXT accepts a string of up to 28 bytes. ${Buffer.byteLength(input, 'utf8')} bytes entered.`);
            }
            break;
        case 'MEMO_HASH':
        case 'MEMO_RETURN':
    ...
    address(input) {
        ...
        // - Includes all valid addresses and a few invalid ones too
        // such as fake TLD or misuse of hyphens or excessive length
